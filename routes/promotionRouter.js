...
.get(cors.cors, (req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
        if (promotion) { //looking for the promotion document/db
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
    ...
    .then(promotion => {
        if (promotion) {
            for (let i = (promotion.comments.length-1); i>=0; i--) {
                promotion.comments.id(promotion.comments[i]).remove();
            }
            promotion.save()
            .then(promotion => {
                res.statusCode = 200;
...
//this handles requests for a specific comment of a specific promotion
promotionRouter.route('/:promotionId/comments/:commentId')
.options(cors.corsWithOptions, (req, res) => res.sendStatus(200))
.get(cors.cors, (req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
        //looking for the promotion document/db & see if the comment is not null
