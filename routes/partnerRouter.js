...
.get(cors.cors, (req, res, next) => {
    Partner.findById(req.params.partnerId)
    .then(partner => {
        if (partner) { //looking for the partner document/db
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
    ...
    .then(partner => {
        if (partner) {
            for (let i = (partner.comments.length-1); i>=0; i--) {
                partner.comments.id(partner.comments[i]).remove();
            }
            partner.save()
            .then(partner => {
                res.statusCode = 200;
...
//this handles requests for a specific comment of a specific partner
partnerRouter.route('/:partnerId/comments/:commentId')
.get((req, res, next) => {
    Partner.findById(req.params.partnerId)
    .then(partner => {
        //looking for the partner document/db & see if the comment is not null
...
.post(cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmin, (req, res) => {
    res.statusCode = 403;
    res.end(`POST operation not supported on /partners/${req.params.partnerId}/comments/${
        req.params.commentId}`);
})
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => { //put updates existing data
    Partner.findById(req.params.partnerId)
    .then(partner => {
         //looking for the partner document/db & see if the comment is not null
        if (partner && partner.comments.id(req.params.commentId)) {
